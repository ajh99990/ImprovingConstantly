...
export function defineService<Id extends string, A extends Api>(id: Id, options: DefineServiceOption<A>) {
  return function (): Service<A> {
    const axiosStatic = options?.axiosStatic || {}
    const axiosStaticObj = isFunction(axiosStatic) ? axiosStatic() : axiosStatic
    let service = inject(`$${id}`) as Service<A>;
    if (service) {
      return service
    }
    service = {} as Service<A>;
    const instance = axios.create(axiosStaticObj) 
    const methods: Array<method> = ["get", "post", "delete", "put", "patch"];
    const env = (process.env.VUE_APP_API_ENV || ServerMode.Development) as ServerMode
    ...
    //避免重复创建实例
    window.global_vue_app_instance.provide(`$${id}`, completeService)
